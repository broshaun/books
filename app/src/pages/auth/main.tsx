...
export const Layout = () => {
    ...
    const drawerMenu = [
        { key: "login", display: true, icon: <Demo label="登录" Icon={IconUserCircle} onClick={() => { navigate({ 'to': "/auth/login" }); close(); }} /> },
        { key: "register", display: true, icon: <Demo label="注册" Icon={IconUserPlus} onClick={() => { navigate({ 'to': "/auth/register" }); close(); }} /> },
    ];
